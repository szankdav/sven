import { AuthorModel, createAuthor, getAuthorByDiscordId } from "../model/author.model.js";
...
export const insertAuthorIntoDatabase = async (db: Database, message: DiscordMessage): Promise<number> => {
    try {
        const messageCreatedAt = new Date(message.messageCreatedAt);
        const authorToCreate: SqlParams = [message.username, message.discordId, messageCreatedAt.toLocaleString()];
        const existingAuthor: AuthorModel | undefined = await getAuthorByDiscordId(db, [message.discordId]);
        if (!existingAuthor) {
...
export const insertMessageIntoDatabase = async (db: Database, message: MessageModel) => {
    try {
        await createMessage(db, [message.authorId, message.content, message.messageCreatedAt]);
...
export const messageLoggerController = async (db: Database, message: DiscordMessage): Promise<void> => {
    try {
        const newAuthorId: number | undefined = await insertAuthorIntoDatabase(db, message);
...
export const createLetterCountersInDatabase = async (db: Database, messageParams: MessageModel): Promise<void> => {
    try {
        const existingAuthorId = await getLetterCounterByAuthorId(db, [messageParams.authorId]);
        if (!existingAuthorId) {
            await createLetterCounters(db, [messageParams.authorId, "", 0, messageParams.messageCreatedAt, new Date().toLocaleString()]);
            logger.info('Letters added for the author!', { authorId: messageParams.authorId });
        }
