import { beforeEach, vi, describe, it, expect, afterEach, MockInstance } from "vitest";
...
describe("messageLogger.controller tests", () => {
    ...
    describe("insertAuthorIntoDatabase tests", async () => {
        it('should add an author to the database from the incoming data', async () => {
            ...
            vi.spyOn(authorModel, 'getAuthorByDiscordId').mockResolvedValueOnce(undefined).mockResolvedValueOnce(testAuthor1);
            ...
            const result = await messageLoggerController.insertAuthorIntoDatabase(db, message);
            expect(result).toBe(1);
            expect(loggerInfo).toHaveBeenCalledWith('Author added to the database!', { username: message.username });
        ...
        it('should return with 0 if there is an error', async () => {
            const testAuthor1: AuthorModel = { id: 1, name: "Teszt Elek", createdAt: createdAtTime };
            vi.spyOn(authorModel, 'createAuthor').mockResolvedValue(undefined);
            vi.spyOn(authorModel, 'getAuthorByDiscordId').mockResolvedValueOnce(undefined).mockResolvedValueOnce(undefined);
            const message: DiscordMessage = {
                username: "Ifj. Teszt Elek",
                discordId: "1069636403335837253",
                messageCreatedAt: 500000000000,
        ...
        it('should not add an author to the database from the incoming data if author already exists', async () => {
            const testAuthor1: AuthorModel = { id: 1, name: "Teszt Elek", createdAt: createdAtTime };
            vi.spyOn(authorModel, 'createAuthor').mockResolvedValue(undefined);
            vi.spyOn(authorModel, 'getAuthorByDiscordId').mockResolvedValueOnce(testAuthor1).mockResolvedValueOnce(testAuthor1);
            ...
            const result = await messageLoggerController.insertAuthorIntoDatabase(db, message);
            expect(result).toBe(1);
            expect(loggerInfo).not.toHaveBeenCalledWith('Author added to the database!', { username: message.username });
            expect(loggerError).not.toHaveBeenCalled();
        })
        ...
        it('should create the message for the newly added author', async () => {
            ...
            vi.spyOn(authorModel, 'getAuthorByDiscordId').mockResolvedValueOnce(testAuthor1).mockResolvedValueOnce(testAuthor1);
            vi.spyOn(messageModel, 'createMessage').mockResolvedValue(undefined);
            vi.spyOn(letterCounterModel, 'getLetterCounterByAuthorId').mockResolvedValue(undefined);
            vi.spyOn(letterCounterModel, 'createLetterCounters').mockResolvedValue(undefined);
            vi.spyOn(letterCounterModel, 'updateLetterCounter').mockResolvedValue(undefined);
            const message: DiscordMessage = {
                username: "Ifj. Teszt Elek",
                discordId: "1069636403335837253",
                messageCreatedAt: 500000000000,
            ...
            const result = await messageLoggerController.messageLoggerController(db, message);
            expect(loggerInfo).toHaveBeenCalledWith('Message added to the database!', { authorId: testAuthor1.id, message: message.content });
            expect(loggerError).not.toHaveBeenCalled();
        })

        it('should throw an error with the correct message', async () => {
            vi.spyOn(messageLoggerController, 'insertAuthorIntoDatabase').mockRejectedValue(new AuthorsError("Error creating author in database:", 500));
            const message: DiscordMessage = {
                username: "Ifj. Teszt Elek",
                discordId: "1069636403335837253",
                messageCreatedAt: 500000000000,
            ...
            await expect(messageLoggerController.insertAuthorIntoDatabase(db, message)).rejects.toThrow(AuthorsError);
            await expect(messageLoggerController.insertAuthorIntoDatabase(db, message)).rejects.toThrow("Error creating author in database:");
        })
        ...
        it('should log an error with the correct message', async () => {
            ...
            await expect(messageLoggerController.insertAuthorIntoDatabase(db, message)).rejects.toThrow("Error creating author in database:");
            expect(loggerError).toHaveBeenCalledWith("Error creating author in database:", error);
        })
    })
    ...
    describe("insertMessageIntoDatabase tests", async () => {
        it('should add a message to the database from the incoming data', async () => {
            ...
            const result = await messageLoggerController.insertMessageIntoDatabase(db, message);
            expect(loggerInfo).toHaveBeenCalledWith('Message added to the database!', { authorId: message.authorId, message: message.content });
            expect(loggerError).not.toHaveBeenCalled();
        })

        it('should throw an error with the correct message', async () => {
            vi.spyOn(messageLoggerController, 'insertMessageIntoDatabase').mockRejectedValue(new MessagesError("Error creating message in database:", 500));
            const message: MessageModel = {
                id: 0,
                authorId: 1,
                content: "Test",
            ...
            await expect(messageLoggerController.insertMessageIntoDatabase(db, message)).rejects.toThrow(MessagesError);
            await expect(messageLoggerController.insertMessageIntoDatabase(db, message)).rejects.toThrow("Error creating message in database:");
        })

        it('should log an error with the correct message', async () => {
            const error = new MessagesError("Error creating message in database:", 500);
            ...
            await expect(messageLoggerController.insertMessageIntoDatabase(db, message)).rejects.toThrow("Error creating message in database:");
            expect(loggerError).toHaveBeenCalledWith("Error creating message in database:", error);
        })
    })
    ...
    describe("messageLoggerController tests", async () => {
        ...
        it('should log an error with the correct message', async () => {
            ...
            vi.spyOn(messageLoggerController, 'messageLoggerController');
            vi.spyOn(messageLoggerController, 'insertAuthorIntoDatabase').mockRejectedValue(error)
            const message: DiscordMessage = {
                username: "Ifj. Teszt Elek",
                discordId: "1069636403335837253",
                messageCreatedAt: 500000000000,
    ...
    describe("createLetterCountersInDatabase tests", async () => {
        it('should create the letters for the newly added author', async () => {
            ...
            const result = await messageLoggerController.createLetterCountersInDatabase(db, message);
            expect(loggerInfo).toHaveBeenCalledWith('Letters added for the author!', { authorId: testAuthor1.id });
            expect(loggerError).not.toHaveBeenCalled();
        })

        it('should not create the letters again for the author if already existed', async () => {
            const testAuthor1: AuthorModel = { id: 1, name: "Teszt Elek", createdAt: createdAtTime };
            vi.spyOn(letterCounterModel, 'getLetterCounterByAuthorId').mockResolvedValue({ authorId: testAuthor1.id });
            vi.spyOn(letterCounterModel, 'createLetterCounters').mockResolvedValue(undefined);
            vi.spyOn(letterCounterModel, 'updateLetterCounter').mockResolvedValue(undefined);
            const message: MessageModel = {
                id: 0,
            ...
            const result = await messageLoggerController.createLetterCountersInDatabase(db, message);
            expect(loggerInfo).not.toHaveBeenCalledWith('Letters added for the author!', { authorId: testAuthor1.id });
            expect(loggerError).not.toHaveBeenCalled();
        })
        ...
        it('should log an error with the correct message if author is not found during getLetterCounterByAuthor', async () => {
            vi.spyOn(messageLoggerController, 'messageLoggerController');
            vi.spyOn(letterCounterModel, 'getLetterCounterByAuthorId').mockRejectedValue(new Error("Author not found!"));
            const message: MessageModel = {
                id: 1,
                authorId: 1,
                content: "",
            ...
            await expect(messageLoggerController.createLetterCountersInDatabase(db, message)).rejects.toThrow("Error creating letters");
            expect(loggerError).toHaveBeenCalled();
        })

        it('should log an error with the correct message if author is not found during createLetterCounters', async () => {
            vi.spyOn(messageLoggerController, 'messageLoggerController');
            vi.spyOn(letterCounterModel, 'createLetterCounters').mockRejectedValue(new Error("Author not found!"));
            const message: MessageModel = {
                id: 1,
                authorId: 1,
                content: "",
