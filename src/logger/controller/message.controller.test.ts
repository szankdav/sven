...
describe("message.controller tests", () => {
    ...
    describe('messagesController tests', () => {
        it('should return with a valid renderObject if data is valid', async () => {
            ...
            const messagesSlicedByTen: MessageModel[] = [testMessage1, testMessage2, testMessage3];
            const authors: AuthorModel[] = [testAuthor1, testAuthor2];
            const error: string = "";
            vi.spyOn(messageModel, 'getAllMessages').mockResolvedValue([testMessage1, testMessage2, testMessage3]);
            vi.spyOn(messageModel, 'getTenMessages').mockResolvedValue([testMessage1, testMessage2, testMessage3]);
            ...
            const result: RenderObject = await messagesController.messagesController(db, 1);
            expect(result.options).toStrictEqual({ messagesPageNumber, authors, messagesSlicedByTen, error });
        })

        it('should return with an error renderObject if data is invalid', async () => {
            vi.spyOn(messagesController, 'messagesController');
            const result: RenderObject = await messagesController.messagesController(db, NaN);
        ...
        it('should return with a valid renderObject if data is not valid', async () => {
            ...
            const messagesSlicedByTen: MessageModel[] = [testMessage1, testMessage2, testMessage3];
            const authors: AuthorModel[] = [testAuthor1, testAuthor2];
            const error: string = "No messages to show... Are you sure you are at the right URL?";
            
            const result: RenderObject = await messagesController.messagesController(db, 2);
        ...
        it('should log an error with the correct message', async () => {
            vi.spyOn(messagesController, 'messagesController');
            vi.spyOn(messageModel, 'getTenMessages').mockRejectedValue(new Error("Error fetching authors!"))
    ...
    describe('messagesByAuthorsController tests', () => {
        it('should return with a valid renderObject if data is valid', async () => {
            const testAuthor1: AuthorModel = { id: 1, name: "Teszt Elek", createdAt: createdAtTime };
            const testMessage1: MessageModel = { id: 1, authorId: 1, content: "Teszt", messageCreatedAt: createdAtTime };
            const testMessage2: MessageModel = { id: 3, authorId: 1, content: "Teszt", messageCreatedAt: createdAtTime };
            const messages: MessageModel[] = [testMessage1, testMessage2];
            const author: AuthorModel = { id: testAuthor1.id, name: testAuthor1.name, createdAt: testAuthor1.createdAt };
            vi.spyOn(authorModel, 'getAuthorById').mockResolvedValue(testAuthor1);
            vi.spyOn(messageModel, 'getMessagesByAuthorId').mockResolvedValue([testMessage1, testMessage2]);
            vi.spyOn(messagesController, 'messagesByAuthorsController');
            const result: RenderObject = await messagesController.messagesByAuthorsController(db, [1]);
        ...
        it('should return with a valid renderObject if data is not valid', async () => {
            ...
            const result: RenderObject = await messagesController.messagesByAuthorsController(db, [10]);
            expect(result.viewName).toBe("author");
            expect(result.options).toStrictEqual({ author, messages });
        })
        ...
        it('should log an error with the correct message', async () => {
            vi.spyOn(messagesController, 'messagesByAuthorsController');
            vi.spyOn(authorModel, 'getAuthorById').mockRejectedValue(new Error("Error fetching author!"))
