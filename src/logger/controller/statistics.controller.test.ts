...
describe("statisticController tests", () => {
    ...
    describe("statisticsByAuthorController tests", () => {
        it("should return with a valid renderObject if data is valid", async () => {
            const author: AuthorModel = { id: 1, name: "Teszt Elek", createdAt: createdAtTime };
            ...
            vi.spyOn(letterCounterModel, 'getLetterCountersByAuthorId').mockResolvedValue(letterCounters);
            const result: RenderObject = await statisticController.statisticsByAuthorController(db, [1]);
            expect(result.options).toStrictEqual({ author, authors, letterCounters, letterStatistics });
        });
    ...
    describe("getLetterStatictics tests", () => {
        ...
        it('should log an error with the correct message', async () => {
            vi.spyOn(statisticController, 'getLetterStatictics');
            vi.spyOn(letterCounterModel, 'getLetterCountersByAuthorId').mockRejectedValue(new Error("Error fetching counters!"))

            await expect(statisticController.getLetterStatictics(db, [1])).rejects.toThrow("Error calculating letter statistics:");
            expect(loggerError).toHaveBeenCalled();
        })
    })
});
